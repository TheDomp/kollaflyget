...
function App() {
  ...
  const toggleType = async () => {
    ...
    if (currentAirport) {
      setLoading(true);
      setSearchTitle(`${newType === 'arrivals' ? 'Ankomster' : 'Avgångar'} vid ${currentAirport}`);
      const data = await swedaviaService.getFlights(currentAirport, newType);
      setFlights(data);
      setLoading(false);
    }
  };
  ...
  return (
    <div className="app-container" style={{ padding: '1rem', minHeight: '100vh' }}>
      <header style={{ textAlign: 'center', margin: '3rem 0' }} className="fade-in">
        ...
        <p style={{ color: 'var(--text-dim)', maxWidth: '400px', margin: '0 auto' }}>
          Utforska ankomster och avgångar vid Swedavias flygplatser med precision och stil. Real-tidsdata direkt från källan.
        </p>
      </header>
      ...
      <main>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', marginBottom: '1rem' }} className="fade-in">
          <button
            ...
            style={{ opacity: type === 'arrivals' ? 1 : 0.6, background: type === 'arrivals' ? 'var(--primary)' : 'var(--glass)' }}
          >
            Ankomster
          </button>
          <button
            ...
            style={{ opacity: type === 'departures' ? 1 : 0.6, background: type === 'departures' ? 'var(--primary)' : 'var(--glass)' }}
          >
            Avgångar
          </button>
        </div>
